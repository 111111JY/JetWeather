package com.example.aiiage.jetweather.step.step.service;
...
import com.example.aiiage.jetweather.StepCountMainActivity;
import com.example.aiiage.jetweather.step.step.UpdateUiCallBack;
import com.example.aiiage.jetweather.step.step.accelerometer.StepCount;
import com.example.aiiage.jetweather.step.step.accelerometer.StepValuePassListener;
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
    ...
    private void updateNotification() {
        //设置点击跳转
        Intent hangIntent = new Intent(this, StepCountMainActivity.class);
    ...
    private void remindNotify() {
        ...
        mBuilder.setContentTitle("今日步数" + CURRENT_STEP + " 步")
                ...
                .setContentIntent(hangPendingIntent)
                .setTicker( "JetWeather提醒您开始锻炼了")//通知首次出现在通知栏，带上升动画效果的
                .setWhen(System.currentTimeMillis())//通知产生的时间，会在通知信息里显示
                .setPriority(Notification.PRIORITY_DEFAULT)//设置该通知优先级
    ...
    private void save() {
        ...
        List<step.step.bean.StepData> list = DbUtils.getQueryByWhere(step.step.bean.StepData.class, "today", new String[]{CURRENT_DATE});
        if (list.size() == 0 || list.isEmpty()) {
            step.step.bean.StepData data = new step.step.bean.StepData();
            data.setToday(CURRENT_DATE);
            data.setStep(tempStep + "");
            DbUtils.insert(data);
        } else if (list.size() == 1) {
            step.step.bean.StepData data = list.get(0);
            data.setStep(tempStep + "");
            DbUtils.update(data);
        } else {
        }
